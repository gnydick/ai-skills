...
| I24 | The gate stays cheap. | Closed check array; no config extension point. Since #73 the array is generated at BUILD time (I43) and the runtime literal is unchanged — still closed, still frozen, still nothing discovered at run time. | 7 | — |
| I25 | Every executed check reports a denominator. | `register_check` always reports N of M through a single `report()` (asserted by the clean-commit test, which also asserts no `citation_target` line appears — a proof line for a leg that did not run would be a false claim). The advisory sweep guard is outside this claim: it is silent when it has nothing to flag — no denominator line. | 6 | Amended 2026-09-02, final review H1: dropped the claimed self-test that would fail a check omitting `report()` — none exists. Amended 2026-09-05, #29: `citation_target` is no longer an executed check; coverage is the one call above. Amended 2026-09-06, #73: `register_check`'s two failure branches that wrote straight to stdout now route through `report()`, so every path out of it — pass, not-staged, stale — carries a denominator; the advisory sweep guard is a declared `blocking: false` check rather than a second execution path, and is still exempt from the denominator claim because it is silent when it has nothing to flag. |
| I26 | Citations are validated once, at authoring. | None since 2026-09-05 (#29): the only mechanism was the citation-target leg, unwired by owner ruling. The module that examined staged-diff citations still exists in the plugin and is tested, but nothing runs it on a commit or a merge, nothing installs it into a project, and since #73 nothing in the tree may claim otherwise (I44). | 0 | Was 6. `--full` (a full-tree sweep) is spec-2 surface, not implemented here (amended 2026-09-02, final review H1); the owner's 2026-09-05 direction is "big sweeps later", anchored on symbol names rather than lines, which is a different tool from this leg. |
...
| I42 | This repo's commit gate stays cheap. | Test budget (20 s) asserted in pre-commit. | 4 | Raised 10 s → 15 s, owner 2026-09-05 ("i'm ok with the increase in budget"): the branch measured 10.4–10.6 s and `date +%s` resolution made the gate fail by phase. Raised 15 s → 20 s, owner 2026-09-07 ("raise it to 20s and merge"): #81 took the suite 622 → 638 tests, measured by alternating rounds at 12.8–12.9 s on main against 14.6–14.9 s on the branch, leaving under a second of headroom. The budget keeps commits quick; it is not a correctness bar, and gating suites behind an env flag was rejected because it weakens what the gate sees on each commit. |
| I43 | The gate's composition is exactly the check modules' own declarations. | `scripts/gate/manifest.mjs` is GENERATED by `scripts/gate-manifest.mjs` from the `declaration` every module under `scripts/gate/` exports, and is the gate's sole source of what to run — blocking and advisory alike, so there is no second execution path beside the array. `node scripts/build-skills.mjs check` regenerates and fails on drift, on a module there that declares nothing, and on a malformed declaration; `install.mjs` copies exactly the manifest's `CHECK_FILES`, so an unwired module cannot reach a project. Spec I24 is preserved exactly: generation happens at BUILD time and the runtime array stays a closed frozen literal with no configuration point. | 6 + 4 | Ceiling: JavaScript has no way to make a hand-typed array unwritable, so 7–8 is unreachable here; the staleness check is what stands in for it, and it is opt-in per clone through `.githooks` — the same hole I7 names. Two structural files (`gate.mjs`, `manifest.mjs`) are exempt from declaring; a third exemption is a visible edit to `NOT_A_CHECK`, asserted by `test/gate-manifest.test.mjs`. |
| I44 | A check cannot be unwired while a claim about it survives. | Every check ships the claims that cite it as `{file, quote}` pairs; `walkClaims()` asserts each quote is still present in the named file and that any check carrying a claim is `wired: true`. Run by the build check and by `test/gate-manifest.test.mjs`, which proves both directions — unwiring a cited check goes red, amending the claim away goes green — and asserts the walker's own denominator, so a run that found nothing is distinguishable from one that found nothing wrong. Rules text asserting a gate blocks counts as a dependent (owner, 2026-09-06). | 4 / 6 | Ceiling, stated plainly: the claim LIST is hand-maintained. A claim whose text has changed or vanished is caught, but a claim DELETED from the declaration in the same commit as the unwiring is not — nothing can see a claim nobody declared. Promotion needs the claim to live at the claim site (a marker in the prose the walker discovers) rather than in a list the check ships. |
