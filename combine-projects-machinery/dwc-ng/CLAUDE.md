...
- **A completion claim records the UAT.** State what was driven, against which
  scenario, and what was observed. No note means the change is not done — a
  claim of completion without this is exactly the "confirmed by reading the
  code" shape the verification-discipline rules above exist to catch, applied
  to UAT instead of diagnosis.
