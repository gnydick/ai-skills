<!-- GENERATED FILE -- DO NOT EDIT.
     Rendered by bash scripts/gen_issue_rules_doc.sh from:
       template:  docs/github-issue-rules.template.md
       authority: docs/rules/tracking-work.md (marker: tracking-work)
     Regenerate: bash scripts/gen_issue_rules_doc.sh
     Hand edits are overwritten by the next regeneration and caught by
     'bash scripts/gen_issue_rules_doc.sh --check', wired blocking in .githooks/pre-commit.
-->

# GitHub issue rules (Ferrislicer)

Source of truth: `docs/rules/tracking-work.md`. This file restates it verbatim and adds the
commands that execute it, so another Claude session can follow it without the repo context.

## Tracking work (verbatim from docs/rules/tracking-work.md)
