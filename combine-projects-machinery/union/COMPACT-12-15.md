...
**Hosted document checks** — triggered by exactly the files their gates read, plus a manual start — kept apart from the checks that build the product and needing none of the build toolchain, because a full build for every documentation edit teaches people to batch documentation edits, which is how a register rots — blocks (ruled: owner, 2026-09-02) — worked when: every gate was measured passing on the commit that introduced it, on the platform it actually runs on (a gate measured elsewhere is noise), and each has a demonstrated way of going red, its own self-test running as a gate beside it. [14.15, 14.16, 14.17, 14.18]
