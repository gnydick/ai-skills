...
// git's stdout as a stream of lines (split on '\n', like git().stdout.split('\n'); a trailing
// newline yields no phantom empty line), holding one chunk and one carried partial line at a
// time — never the whole output (ticket #19, owner: "can't we operate on a stream?"). The
// returned object is async-iterable and carries the child's `pid` (to see the process) and
// `kill()` (to end it through its own handle: measured 2026-09-05, on Windows a kill from
// OUTSIDE is a TerminateProcess that Node reports as exit 1 with no signal — still a rejection,
// but with no signal to name). Completion is judged only once the child has CLOSED: a non-zero
// exit, a signal, or a spawn failure rejects the iteration, naming the command, the exit code
// or signal, and git's own stderr — so a truncated output can never read as a complete one.
// Stopping the iteration early kills the child rather than leaving it blocked on the pipe.
export function gitLines(args, cwd) {
  const child = spawn(gitExe(), args, { cwd, stdio: ['ignore', 'pipe', 'pipe'], env: spawnEnv() });
  let spawnError = null;
  let stderr = '';
  ...
  return { pid: child.pid, kill: () => child.kill(), [Symbol.asyncIterator]: lines };
}
