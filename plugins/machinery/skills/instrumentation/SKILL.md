...
- A new diagnostic switch is inert until turned on, prints its denominator and explicit zeros, matches its shape to the question being asked, and is added to the project's diagnostics index in the same change.
...
- Any instrument built to verify a change ships in the same build as the change. Verifying one build with an instrument compiled into another is comparing two things, not measuring one.
