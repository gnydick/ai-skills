...
3. Sweep for the old locations built in PIECES, before you apply. The plan's `references` list comes from `git grep -F`, so it finds literal strings only — `docs/adr` written out in one piece. It cannot see `os.path.join(REPO, "docs", "adr")`, a `Path("docs") / "adr"`, a constant holding `adr` on its own, or any path assembled at run time. Measured on ferrislicer, 2026-09-19: `scripts/adr_provenance_gate.py` built its path that way, the sweep missed it, the migration landed, and that project's CI gate went red on the migration's own commit.
   - Search for the directory name alone, not only the whole path: `git grep -n adr` and `git grep -n dictated-specs`. Read every hit that is code rather than prose, and open any file whose plan entry shows only comments and docstrings — the code around them may build the path in pieces.
   - Add each file you find to `references` with explicit `replace` pairs that rewrite those pieces, and `reviewed: true`. A pair is a plain string swap, so choose strings that appear only where you mean them.
   - Say in your report what you swept and what the sweep cannot reach. Nothing here proves the list is complete: a path assembled at run time is invisible to both `git grep` and the plan.
