...
- The `SPEC:` mark is captured word for word to the project's spec inbox before the assistant replies, from an isolated working copy as readily as from the root, exactly as a rule mark is. An undispositioned spec entry blocks commits, exactly as an undispositioned rule entry does.
- Dictated specifications persist under `docs/dictated-specs` — one fixed, known location every project shares, with nothing to declare, resolve or guess; a disposition names the path the specification was filed to, and the gate refuses a disposition naming a path outside that area. Which specification file owns a given subsystem stays a judgement no mechanism makes — the area, and that the filing lands inside it, are what is enforced. Nothing loads a filed specification into a session yet: `docs/` is outside `.claude/`, so reaching a session is separate work and is not claimed here.
